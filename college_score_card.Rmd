...
This is the temporary dataset that contains the columns which I will work on in my analysis. 
```{r}
mean_earning <- college %>%
  select(MN_EARN_WNE_MALE0_P10, MN_EARN_WNE_MALE1_P10,
         MN_EARN_WNE_MALE0_P6, MN_EARN_WNE_MALE1_P6)
```

### Rename the Columns
The columns are renamed to be more readable.
```{r}
mean_earning_renamed <- mean_earning %>%
rename(mean_earning_female_10yrs = MN_EARN_WNE_MALE0_P10,
...
Since these four columns are numerical variables, they do not need to be recoded. 

### Reshape the Columns
An additional column `gender` is created to label the observations with either "female" or "male". 
```{r}
# create the female columns
female <- mean_earning_renamed %>%
  select(mean_earning_female_10yrs, mean_earning_female_6yrs) %>%
  mutate(gender = "female")
# create the male columns
male <- mean_earning_renamed %>%
  select(mean_earning_male_10yrs, mean_earning_male_6yrs) %>%
  mutate(gender = "male")
```

In order to analyze the income gap between the different sex, the columns for mean-earning after 10 year and mean-earning after 6 years are combined by rows. 
But first, the `mean_earning_female_10yrs` and `mean_earning_male_10yrs` columns in `female` and `male` are renamed to `mean_earning_10yrs`. The same will be done to the data for mean-earning after 6 years, renaming them to `mean_earning_6yrs`. 
```{r}
...
The two datasets `female_renamed` and `male_renamed` are then row combinded to `mean_earning_clean` which will be used for the data analysis in the next section. 
```{r}
...
First, the columns needed for this question are selected to store in `mean_earning_clean`.
```{r}
mean_earning_clean <- college_clean %>%
  select(mean_earning_6yrs, mean_earning_10yrs, gender)
```
...
**Histograms and PMF of the mean-earning after 6 years:**
```{r}
...
**Histograms and PMF of the mean-earning after 10 years:**
```{r}
ggplot(mean_earning_clean, 
...
**Summary statistics for mean-earning after 10 years:**
```{r}
...
```
The summary statistics suggest that an income gap does exists between female and male graduates. In 6 years of graduation, the median income of male graduates is \$6200 higher than that of female graduates. In 10 years of graduation, the median income of male graduates is \$10950 higher than that of female graduates. There is an increase in the median income gap of \$4750, or 76.6%, between male and female graduates from 6 to 10 years of graduation. 
...
First, a hypothesis testing for the mean-earning of college graduates after 6 years of graduation is conducted.  
...
pvalue_6yrs
```
Since the p-value of 0 is less than the significance level of 0.05, the null hypothesis is rejected in favor of the alternative hypothesis. Therefore, there is a difference between the median mean-earning for the male and female college graduates after 6 years of graduation. 


Similarly, a hypothesis testing with a significance level of 0.05 is also conducted for the mean-earning for college graduates after 10 years of graduation.
...
pvalue_10yrs
```
Since the p-value of 0 is less than the significance level of 0.05, the null hypothesis is rejected in favor of the alternative hypothesis. Therefore, there is a difference between the median mean-earning of the male and female college graduates after 10 years of graduation. 

Both the hypothesis tests suggest that the median mean-earning of male and female college graduates differ. This means there exists an income gap where male graduates have a higher median mean-earning than female graduates after 6 and 10 years of graduation. 
...
The effsize library needs to be loaded to carry out this test. 
```{r}
library(effsize)
```
...
```
The Q-Q plots show that the data do not have a normal distribution which is the condition of using the Cohen's d test. In order to satisfy the condition of a normal distribution for a valid Cohen's d test, the mean-earning columns are transformed to a logarithmic scale of base 10 using `log10()`. Next, create the Q-Q plots for the transformed mean-earning columns to ensure their normal distribution. 
```{r}
ggplot(mean_earning_clean) +
...
cohen.d(formula = log10(mean_earning_6yrs) ~ gender, data = mean_earning_clean, method = "pooled")
        
# calculation for mean-earning after 10 years
cohen.d(formula = log10(mean_earning_10yrs) ~ gender, data = mean_earning_clean, method = "pooled")
```
The result of the Cohen'd calculation shows that the large effect size of the difference between the mean of the mean-earning after 10 years of graduation is greater than the medium effect size of that after 6 years of graduation for the male and female graduates. Therefore, the income gap has widen after 10 years of graduation as compared to 6 years of graduation. 
